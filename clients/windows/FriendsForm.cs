...
internal sealed class FriendsForm : UserControl
{
    ...
    internal void SetDevice(string? reference){if(reference is not null)device.Text=T("Ключ устройства создан · ID ","Device key created · ID ")+reference;}
    readonly TextBox invitation=new(){Multiline=true,Height=70,MaxLength=256,PlaceholderText="https://185.251.89.19:8443/invite/#…"};
    ...
    internal FriendsForm(bool russian,Func<Request,Task<Reply>>? caller=null)
    {
        ...
        register.Click+=async(_,_)=>{
            if(invitation.Text.Trim().Length>0){
                var match=System.Text.RegularExpressions.Regex.Match(invitation.Text.Trim(),@"\A(?:https://185\.251\.89\.19:8443/invite/#|familyconnect://invite/)([0-9a-f]{64})\z");
                if(!match.Success){status.Text=T("Вставьте полную ссылку приглашения, полученную в приложении на телефоне.","Paste the full invitation link from the app on your phone.");return;}
                PendingInvitation=match.Groups[1].Value;
            }
            await Run("friends-register");
        };
    ...
    internal void SetLanguage(bool russian){
        ru=russian;hint.Text=T("Откройте исходную ссылку приглашения и нажмите «Открыть приложение». Или вставьте полную ссылку ниже. Ключ создаётся на этом компьютере автоматически.","Open your original invitation link and choose Open app. Or paste the full link below. The key is created automatically on this PC.");
        register.Text=T("Активировать доступ","Activate access");share.Text=T("Пригласить друга","Invite a friend");copy.Text=T("Скопировать ссылку","Copy link");
    }
    async Task Run(string action)
    {
        ...
        try {
            ...
            if(action=="friends-register"){
                PendingInvitation="";invitation.Clear();status.Text=T("Доступ готов. Подключитесь на главном экране.","Access is ready. Connect on the main screen.");SetDevice(reply.Device);RegistrationChanged?.Invoke();
            }else{
    ...
    internal static string Error(string? code,bool ru)=>code switch{
        "access_rejected"=>ru?"Откройте ссылку приглашения и нажмите «Открыть приложение». Если доступ отозван — обратитесь к владельцу.":"Open an invitation link or paste it above. If access was revoked, contact the owner.",
    ...
    internal static void CheckUi()
    {
        foreach(bool ru in new[]{false,true}){
            var calls=new List<string>();string? activation=null;
            using var host=new Form{ClientSize=new(400,540)};
            using var page=new FriendsForm(ru,r=>{calls.Add(r.Action);if(r.Action=="friends-register")activation=r.Activation;return Task.FromResult(new Reply(true,"inactive",Code:r.Action=="friends-referral"?"https://185.251.89.19:8443/invite/#"+new string('a',64):null,FriendsReady:true));});
            host.Controls.Add(page);host.Show();Application.DoEvents();
            page.invitation.Text="https://185.251.89.19:8443/invite/#"+new string('b',64);
            page.register.PerformClick();Application.DoEvents();
            if(calls.Count!=1||calls[0]!="friends-register"||!page.register.Enabled||activation!=new string('b',64)||page.invitation.Text.Length!=0)throw new Exception("Inline registration failed");
            page.invitation.Text="not an invitation";page.register.PerformClick();Application.DoEvents();
            if(calls.Count!=1)throw new Exception("Malformed invitation reached broker");page.invitation.Clear();
