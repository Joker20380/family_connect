...
final class ControlEnrollmentEnvelope {
    static final int OVERHEAD = 29;
    private static final byte[] AAD = "family-connect/control-enrollment/v1".getBytes(StandardCharsets.US_ASCII);
    static byte[] seal(SecretKey key, byte[] plain) throws GeneralSecurityException {
        if (plain.length == 0 || plain.length > 16384) throw new GeneralSecurityException("Invalid journal size");
    ...
    static byte[] open(SecretKey key, byte[] raw) throws GeneralSecurityException {
        if (raw.length <= OVERHEAD || raw.length > 16384 + OVERHEAD || raw[0] != 1)
