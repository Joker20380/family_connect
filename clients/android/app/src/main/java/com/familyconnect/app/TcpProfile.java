...
final class TcpProfile {
    ...
    private static final Set<String> XHTTP=new HashSet<>(Arrays.asList("type","server","port","id","server_name","path","mode"));
    ...
    static Map<String,String> parse(String raw)throws Exception{
        ...
        while(pos<body.length()){
            ...
            require((FIELDS.contains(key)||XHTTP.contains(key))&&!p.containsKey(key));require(key.equals("port")?m.group(3)!=null:m.group(2)!=null);
        ...
        boolean xhttp="vless-xhttp-tls-v1".equals(p.get("type"));
        require(p.keySet().equals(xhttp?XHTTP:FIELDS)&&(xhttp||"vless-reality-v1".equals(p.get("type"))));
        ...
        if(xhttp){
            require("packet-up".equals(p.get("mode"))&&p.get("path").matches("/[A-Za-z0-9_-]{16,128}/"));
        }else{
            ...
            require(p.get("short_id").matches("(?:[0-9a-f]{2}){1,8}"));
        }return p;
    }
    static String validate(String raw)throws Exception{
        Map<String,String> p=parse(raw);StringBuilder out=new StringBuilder("{");
        for(String key:p.get("type").equals("vless-xhttp-tls-v1")?new String[]{"type","server","port","id","server_name","path","mode"}:new String[]{"type","server","port","id","public_key","server_name","short_id"}){
