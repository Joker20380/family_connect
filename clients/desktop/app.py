...
class App:
    def __init__(self,root,smoke=False):
        ...
        root.withdraw()
        self.scale=max(1.0,float(root.tk.call('tk','scaling'))/(96/72),tkfont.Font(root=root,font=('Segoe UI',11)).metrics('linespace')/18)
        root.title(f'Family Connect · {APP_VERSION}');root.minsize(360,420);root.configure(bg='#0b1020')
        ...
        style.configure('FC.TCombobox',background='#0b1020',borderwidth=0)
        style.map('FC.TCombobox',background=[('readonly','#0b1020'),('disabled','#0b1020')])
        ...
        root.option_add('*TCombobox*Listbox.selectBackground','#a5b4fc');root.option_add('*TCombobox*Listbox.selectForeground','#0b1020')
        root.option_add('*TCombobox*Listbox.font',('Segoe UI',11));root.option_add('*TCombobox*Listbox.borderWidth',0)
        root.option_add('*TCombobox*Listbox.highlightThickness',0)
        style.layout('FC.Vertical.TScrollbar',[('Vertical.Scrollbar.trough',{'sticky':'ns','children':[('Vertical.Scrollbar.thumb',{'sticky':'nswe'})]})])
        ...
        self.paint();self.fit_initial_window();self.drain_timer=root.after(100,self.drain)
        if not smoke:
            self.register_icon()
    ...
    def confirm(self,text,action):
        ...
        width=min(max(380,round(430*self.scale)),self.root.winfo_screenwidth()-64)
        body=tk.Frame(dialog,bg='#0b1020',padx=24,pady=24);body.pack(fill='both',expand=True)
        tk.Label(body,text='Family Connect',bg='#0b1020',fg='#a5b4fc',font=('Segoe UI',17,'bold')).pack(anchor='w',pady=(0,16))
        tk.Label(body,text=text,bg='#0b1020',fg='#e8edff',font=('Segoe UI',11),justify='left',wraplength=width-48).pack(fill='x',pady=(0,20))
        result=False
        def finish(answer):
            nonlocal result
            result=answer;dialog.destroy()
        buttons=tk.Frame(body,bg='#0b1020');buttons.pack(fill='x')
        yes=ttk.Button(buttons,text=action,style='FC.TButton',command=lambda:finish(True));yes.pack(fill='x',pady=(0,8))
        cancel=ttk.Button(buttons,text='Отмена' if self.ru else 'Cancel',style='FC.TButton',command=lambda:finish(False));cancel.pack(fill='x')
        dialog.protocol('WM_DELETE_WINDOW',lambda:finish(False));dialog.bind('<Escape>',lambda _:finish(False))
        dialog.update_idletasks();height=dialog.winfo_reqheight()
        x=max(0,min(self.root.winfo_rootx()+(self.root.winfo_width()-width)//2,dialog.winfo_screenwidth()-width))
        y=max(0,min(self.root.winfo_rooty()+(self.root.winfo_height()-height)//2,dialog.winfo_screenheight()-height-48))
        dialog.geometry(f'{width}x{height}+{x}+{y}');dialog.deiconify();dialog.grab_set();cancel.focus_set()
        self.root.wait_window(dialog);self.root.focus_set();return result
    def register_icon(self):
        ...
        try:
            ...
            if not (root/'app.png').is_file() or (root/'app.png').read_bytes()!=icon:atomic(root/'app.png',icon)
    ...
    def paint(self):
        ...
        if self.driver is None:self.setup.grid()
        else:self.setup.grid_remove()
        if self.detail['text']:self.detail.grid()
        ...
        if self.layout_timer is None:self.layout_timer=self.root.after_idle(self.scheduled_layout)
    def scheduled_layout(self):
        self.layout_timer=None
        if not self.closed:self.layout()
    def layout(self,event=None):
        ...
        key=(minimum,width,needed)
        if self.layout_key==key:return
        self.layout_key=key
    ...
    def refresh(self):
        ...
        future=self.poll_pool.submit(poll)
    ...
    def update_application(self):
        ...
        if not self.confirm(question,'Установить обновление' if self.ru else 'Install update'):return
        def install():
