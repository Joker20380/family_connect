...
не подключён к startup/enrollment/UI/carrier, Android runtime ещё не принят.
[Текущий checkpoint](releases/2026-09-14-android-control-application.ru.md).

14.09.2026: legacy ConnectionService/ProfileStore теперь используют общий owner;
active lease блокирует profile edits и независимый control receive/recover.
Persisted managed state отказывает legacy mutation до native recovery wiring.
33 JVM tests и app Java compilation passed, actual Android runtime ещё не принят.
[Текущий checkpoint](releases/2026-09-14-android-control-owner.ru.md).

На14.09.2026 Android journal/transaction и encrypted storage adapter реализованы
в исходниках;23 Java tests, включая16 шагов Python transcripts, прошли. Реального
service binding и runtime проверки storage пока нет. Следующий шаг — общий owner
с Activity/ConnectionService/ProfileStore и process-death recovery до mutation.
[Точный checkpoint и ограничения](releases/2026-09-14-android-control-journal.ru.md).
Мессенджер/iOS отложены до всех [условий выпуска трёх платформ](PLAN.md#release-gates-three-platforms).

Android identity block подготовлен13.09.2026: отдельный Keystore vault и совместимый
Python enrollment proof,8 Java/51 Python tests. Runtime vault ещё не принят;
journal/outbox/ownership/recovery остаются следующими. См.
[актуальный checkpoint](releases/2026-09-13-android-control-identity.ru.md).

Android verifier принят в API35 x86_64 emulator на2415920, Clients34750425107 passed.
