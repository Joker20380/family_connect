...
## Обновления, язык, бренд и карта — актуально 2026-09-20

Android **0.1.18-beta35/code35** установлен и сверён по SHA256; 147 unit,
lint/build и 5 device tests passed. Компактный круг, график слева/соединение справа,
нижняя карта с точками начала/конца, без прямоугольника телефона и его подписи.
Ручной updater: HTTPS discovery, проверка APK/сертификата, системная установка;
реальный system-install consent нового APK ещё не пройден (установка была adb -r).
Бренд family_connect/monospace и сохранение языка внедрены во всех клиентах.
Linux0.2.9 bundle146d221d0ad23c07 установлен; 24 layouts passed.
Windowsa6c68fe: installer/UI/AWG/TCP CI passed, физический ПК недоступен.
Страница /invite/ и Android catalog обновлены: beta35/Linux146d/Windowsa6c68fe.
Desktop signed catalogs не менялись. Источники Android остаются в исходном рабочем
дереве и локальном snapshot beta35; desktop SHA не описывает APK.
Далее: подготовить безопасный апгрейд NL186.246.45.246 после уточнения тарифа/IP;
пользователь сообщил порт «до 1 Гбит/с», прежние200 Мбит/с не подтверждены для VPS.
Апгрейд пока не выполнялся. Windows/российская сеть, устойчивость и независимая
доставка конфигураций остаются открытыми; глобальный этап4 не закрыт.
[Точные версии, проверки и откат](releases/2026-09-20-updater-language-brand.ru.md).


## Шкала нагрузки на всех клиентах — актуально 2026-09-20

Linux0.2.9 preview3ef22e124f177f82 установлен. Android0.1.18-beta26/code26
установлен с сохранением данных; 143 unit +4 device UI tests passed.
Windows a1be617: installer/UI/AWG CI passed, физический ПК недоступен.
Повтор TCP CI35506712715 после исправления выбора порта тестового стенда в работе.
RU/NL мониторинг считает нагрузку по CPU и исходящему каналу200 Мбит/с — оценке
по общим условиям провайдера, с «≈»; тарифы VPS не подтверждены.
Публичная /invite/: beta26/Linux3ef22e/Windowsa1be617; все файлы скачаны и SHA256
сверены, CSP/старые URL сохранены. Подписанный update catalog не менялся.
Следующая отдельная задача: Android «Проверить обновления», проверенная загрузка APK
и системное подтверждение установки. Сейчас встроенного updater нет.
Этап4 не закрыт: Windows-ПК, российская сеть, длительная устойчивость, Linux standalone.
[Версии, проверки, ограничения и откат](releases/2026-09-20-server-load-platforms.ru.md).



## Linux: шкала нагрузки и серверные метрики — 2026-09-20

Установлен Linux0.2.9 preview c55149740c24618c/source b4ef324: вместо блока
кнопки/переключателя под кругом — тонкая шкала. Круг управляет подключением.
Мониторинг RU/NL работает; CPU и RX/TX реальные. Лимиты каналов пользователю
неизвестны, capacity_mbps=null, общий процент честно «Нет данных»; детали по наведению.
12 targeted tests,24 layouts,Cairo load/map/dial checks passed; Linux control
CI35504074694/job106060785918 success. GUI active, живое чтение обоих gateway проверено.
VPN-службы не перезапускались. Публичный Linux download пока11c53d18c3bc10a9;
Android/Windows и каталоги автообновления прежние.
Далее: оценка UI; подтверждение ёмкости, затем общий процент; перенос на остальные
клиенты и отдельная публикация новой Linux-сборки остаются открытыми.
[Версии, проверки, мониторинг и откат](releases/2026-09-20-server-load.ru.md).


## Три загрузки и общая Linux-навигация — 2026-09-20

Страница /invite/ обновлена: Android0.1.18-beta25, Linux0.2.9 paired preview
11c53d18c3bc10a9/source d93611e, Windows0.2.9 preview21b740a.
Linux установлен: приглашения внутри основной области, общая шапка/нижние вкладки,
выход через Esc/вкладки/крестик. GTK interaction и24 layouts passed;
Linux control CI35503217768 success, SHA архива совпал с CI.
Windows UI/installer jobs приняты; физический ПК пока недоступен.
Linux download — ручной архив с зависимостями; standalone installer остаётся задачей.
Beta13 universal сохранена по старому URL, убрана из основного списка.
Каталоги автообновления не менялись. Публичные файлы скачаны целиком: размеры и
SHA256 совпали, HTML/CSP и сохранение старого URL проверены.
[Публикация и откат](releases/2026-09-20-three-platform-downloads.ru.md).
[Linux исправление](releases/2026-09-20-linux-invitation-navigation.ru.md).

Далее: оставшиеся проверки этапа4; обсуждение [масштабирования серверов](server-capacity-plan.ru.md).
Это проект, не внедрённая балансировка; текущий каталог ограничен RU/NL.
Новые серверы/нагрузочные тесты не запускались. Анимация отдельно.



## Desktop: карта Android beta25 и интерактивный круг — 2026-09-20

Пользователь принял карту Android beta25 и попросил тот же вид на остальных
платформах, затем уточнил перенос круга на главной. Linux установлен и запущен:
...
Windows-рендеры карты и круга просмотрены. Следующее: пользовательская оценка Linux,
затем отдельная художественная доработка анимации. QR5002579 также входит в новый
установленный Linux bundle; helpers7a8036a не менялись.
[Точные версии, проверки, установка и откат](releases/2026-09-20-desktop-map-dial.ru.md).


## Android beta25: одинаковые точки по референсу — 2026-09-20

Установлена 0.1.18-beta25/code25 поверх beta24. По последнему запросу пользователя
случайное распределение заменено ровными точками с шахматным смещением рядов.
Все точки имеют общую яркость; центры и диаметр привязаны к физическим пикселям,
чтобы сглаживание не делало часть точек темнее. Лёгкое общее мерцание сохранено.
140 unit tests и 4 native UI tests passed; lint 0 errors / 18 warnings.
Версия и SHA256 установленного APK сверены; экран «Маршрут» открыт для оценки.
Следующее: пользовательская оценка карты, затем отдельная работа над анимацией.
Public beta19, Linux и отложенная Windows-приёмка без изменений.
[Артефакт, проверки и rollback](releases/2026-09-20-android-map-beta25.ru.md).


## Android beta24: равномерная фактура карты и закреплённая дужка — 2026-09-20

Установлена 0.1.18-beta24/code24 поверх beta23 без удаления данных.
По замечаниям пользователя исправлен открытый замок: правая сторона дужки
всегда соединена с корпусом. Карта использует нерегулярное распределение
с минимальным расстоянием 1.30dp вместо сдвинутой решётки; точки 0.58dp,
мерцание уменьшено. 140 unit tests и 4 native UI tests passed;
lint 0 errors / 18 warnings. Версия и SHA256 установленного APK сверены.
Следующее: пользовательская оценка фактуры карты и круга, затем работа над анимацией.
Public beta19, Linux и отложенная Windows-приёмка без изменений.
[Проверки, артефакт и rollback](releases/2026-09-20-android-map-beta24.ru.md).


## Android beta23: карта и интерактивный круг по референсу — 2026-09-20

Установлена 0.1.18-beta23/code23 на Redmi Note 9 Pro, данные сохранены.
Карта мира: точки 0.65dp с шагом 1.84dp (примерно вдвое плотнее beta21),
нерегулярное расположение, тихая сетка и мерцание. Общий clip маршрута сохранён.
Круг на главной и в «Маршрут»: сегменты, тонкие кольца, риски, орбитальные маркеры;
нажатие вызывает существующее подключение/отключение, состояние берётся из приложения.
140 unit tests и 4 native UI tests passed; lint: 0 errors, 18 warnings.
Установленные версия и SHA256 проверены. Public beta19 не менялась.
Следующее: пользовательская оценка нового вида и отдельная доработка анимации;
живое VPN-подключение новым кругом пока не проверено. Windows ждёт доступного ПК.
Linux 5a9ca444046993a7 прежний, QR preview 9c4fb610ef3153ec ещё не установлен.
[Изменения, проверка, артефакты и rollback](releases/2026-09-20-android-reference-beta23.ru.md).


## Android beta21: более тонкая нерегулярная фактура суши — 2026-09-20

По просьбе пользователя диаметр точек уменьшен1.15→0.65dp, средний шаг4→2.6dp
(плотность примерно×2.37). Независимые постоянные смещения по обеим осям убирают
горизонтальные/вертикальные ряды. Fixed seed сохраняет расположение при мерцании.
Grid16dp и clip/жесты beta20 сохранены. Beta21/code21 установлена поверхbeta20,
данные не удалялись.140 unit tests и2 native Canvas tests passed, lint0errors/16warnings.
SHA256 установленного APK сверена:eb3228b91b9de1bb3fbb7d244bc1251da9a7b77ba48adb691e317d9afdabafad.
Рендер просмотрен, карта открыта для оценки. Public beta19 прежняя; Windows отложен.
[Артефакт, рендер и rollback](releases/2026-09-20-android-map-beta21.ru.md).
...
сохраняем открытой; не запрашивать повторную установку до возвращения устройства.
Следующий приоритет пользователя — Android RouteMapView: маршрут не должен выходить
за рамку при zoom/pan; суша из мелких слегка мерцающих точек, более мелкая сетка.
Локальная реализация и Canvas-регрессии подготовлены; установленная beta19 прежняя.
Beta20/code20 собрана и подписана:140 unit tests passed, lint0errors/16warnings.
Телефон подключён, установленная beta19 с активным VPN; установка ждёт штатного
отключения пользователем. Native Canvas runtime pending. Финальная сборка/экранная
приёмка фиксируются в [отчёте карты](releases/2026-09-20-android-map.ru.md).

Параллельная законченная доработка исходников Linux: QR ссылки приглашения.
8 targeted tests passed, независимый ZBar декодер восстановил точную ссылку;
GTK QR/close и Friends parent checks passed, рендер просмотрен. QR локальный,
без стороннего HTTP/файлов с реальными ссылками. Новый bundle9c4fb610ef3153ec
подготовлен, но не установлен: рабочий Linux остаётся5a9ca444046993a7.
[QR изменения и rollout](releases/2026-09-20-linux-invitation-qr.ru.md).
