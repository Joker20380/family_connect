> Архив до пересмотра документации 23.09.2026. Текущие факты: [STATUS](STATUS.md), задачи: [PLAN](PLAN.md).

# Working plan / Рабочий план

## Голосовые и прокрутка — beta49 опубликована, 2026-09-23

Beta49/code49 установлена поверх прежней без очистки и опубликована в Android discovery.
Удержание микрофона записывает, отпускание отправляет; свайп вверх фиксирует запись,
влево отменяет. Пользователь подтвердил запись/доставку обоими способами на beta48.
Beta49 постоянно показывает кнопку текстовой отправки и после нового сообщения
открывает последнюю страницу с прокруткой после layout; обновления статусов сохраняют
позицию чтения. Android153 unit/lint/build passed; native gesture/scroll проверки
passed, synthetic renders просмотрены. Публичные APK size/SHA256/discovery проверены.
Пользователь подтвердил уведомление при выключенном экране. Bounded Doze: система
подтвердила IDLE около122с, затем ACTIVE, ChatDeliveryService сохранилась; unforce и
battery reset выполнены. Пользователь подтвердил вибрацию уведомления без включения экрана,
но время относительно выхода из IDLE он не заметил: задержка в Doze не оценена. Android13+, другие OEM,
длительный сон и boot остаются открытыми; затем общий выпуск клиентов по плану.
[Версии, проверки и откат](releases/2026-09-23-voice-scroll-beta49.ru.md).

## Голосовые: отправка и новый вид — beta47, 2026-09-23

Исправлены два подтверждённых отказа: Android Ogg без EOS (beta46) и старый лимит
Java encrypted Store64КиБ при сохранении relay_blob (beta47, bounded row1МиБ).
Beta47 установлена поверх прежней без очистки и опубликована в Android discovery.
Пользователь подтвердил: застрявшее голосовое дошло на второй телефон и воспроизводится.
Native обмен: error=null/failures0/outbox_pending0. Голосовое отображается компактным
блоком с play/pause, длительностью, реальным прогрессом и перемоткой; волна декоративная.
Python65+1, Java AES-GCM large-record/bounds, Android147 unit/lint/build passed.
Native play/pause/seek/render passed, synthetic render просмотрен. Тестовый пакет
удалён; обычный запуск и ChatDeliveryService проверены, installed SHA256 совпал.
VPN был выключен до обновления и не включался. Сервер сообщений не менялся.
Публикация: public APK hash/size/discovery проверены серверным installer и отдельным
скачиванием с внешнего компьютера. Backup discovery/nginx: backup-android47-v2.
Далее: обновить второй Android для отправки и длинных голосовых; длительный фон/Doze,
OEM/Android13+ и общий desktop выпуск остаются открытыми.
[Версии, native доказательства и откат](releases/2026-09-23-voice-send-beta46.ru.md).

## Исправление отправки голосовых — beta46 подготовлена, 2026-09-23

Пользователь сообщил: голосовое не отправляется, ошибка «Проверьте публичный ключ
или введённые данные». Воспроизведён отказ валидатора на полных Ogg/Opus страницах
без EOS, которые может создавать Android OggWriter. Перед подписью отправитель
проверяет CRC/целостность и завершает последнюю страницу; приёмник остаётся строгим.
Для ошибки аудио теперь отдельное сообщение. Beta46/code46 собрана и подписана
прежним сертификатом; **не установлена и не опубликована**. Публичная beta45 сохранена.
Python65 +1 новый Android-carrier regression passed; Android147 unit/lint/build,
APK/ABI/подпись/версия/alignment passed. Первую сборку с Python3.14 отклонила
проверка APK; итоговая пересобрана с Python3.10 и прошла проверку.
ADB дважды не обнаружил подключённых устройств. Пользователь имеет два телефона.
Далее: подключить телефон, установить beta46 поверх beta45 без очистки, проверить
реальную запись/отправку/воспроизведение на втором Android beta45+, затем публикацию.
Микрофонный случай пользователя ещё не подтверждён как исправленный.
[Исправление и оставшаяся приёмка](releases/2026-09-23-voice-send-beta46.ru.md).

## Редактирование и голосовые — beta45, 2026-09-23

Добавлены список/просмотр/правка объявлений для администратора, правка своего текста
долгим нажатием в личном чате и голосовые Opus (моно12кбит/с, до60с, записьAndroid10+).
Редакции объявлений — в v2 feed, исходная v1 сохранена для старых клиентов.
Обоим собеседникам нужна beta45 для правок/аудио. Приватные временные записи удаляются;
история и пересылка зашифрованы. Сервисные объявления не редактировались тестами.
Python150 +11 targeted (включая минутное аудио через реальный локальный RNS) passed,
Android147 unit/build/lint passed; native Opus/list/editor-preview/role/import/notification
проверены. Beta45 установлена и опубликована в Android discovery. Ещё3 native проверки
публичного updater/подписи/VPN passed; обычное VPN-соединение восстановлено,
ChatDeliveryService жив, установленный SHA256 совпал.
Реальная запись с микрофона/второй Android, Doze/OEM и desktop остаются в следующей приёмке.
[Реализация, версии и откат](releases/2026-09-23-chat-edit-voice.ru.md).


## Задержка объявлений — beta44, 2026-09-23

Подтверждён интервал300с в beta43: сообщение было на сервере и позже пришло второму
Android. Beta44 загружает ленту при открытии/после публикации, проверяет фон раз в30с
и обновляет открытый диалог независимо от личного mailbox. Установлена на телефоне;
Python16, Android147 unit/build/lint и8 native tests passed. Beta44 опубликована; далее beta45.
[Причина, версии, проверки и откат](releases/2026-09-23-events-delay.ru.md).


## Администратор на телефоне и Android beta43 — 2026-09-23

Подключённому телефону назначена роль администратора объявлений. Подписанный запрос
с устройства и экран «Вы администратор» проверены; вход: Мессенджер → Family Connect → ⋮.
Beta43 установлена с сохранением данных и **опубликована в Android update discovery**.
Второе устройство с beta35 теперь может обновиться через настройки. Публичные APK,
SHA256/подпись и live check проверены с телефона и внешнего компьютера.
Python77, Android147 unit/build/lint,7 native UI/notification/admin tests и3 updater/VPN
passed. Объявления не рассылались. Обычный VPN восстановлен после instrumentation; живые VPN/chat службы и установленный SHA256 проверены.
Далее: длительный фон/Doze/два устройства, Android13+/OEM/boot; синхронизация
invite/README загрузок с новой версией и согласованный выпуск desktop остаются открытыми.
[Версии, проверки и откат](releases/2026-09-23-device-admin.ru.md).

## Мессенджер и глобальный план — 2026-09-23

По новому указанию пользователя текущий приоритет — фоновое получение сообщений,
системные уведомления, служебные объявления владельца/назначенных администраторов
и локальные имена собеседников. Это не закрывает незавершённый выпуск VPN-клиентов.

| Этап | Текущее состояние |
| --- | --- |
| 1. Базовый продукт/WG/идентичности | Работает в пилоте; выдача и отзыв доступа реализованы |
| 2. AWG и восстановление | AWG3.1 работает; длительная устойчивость и российская сеть ещё не приняты |
| 3. TCP REALITY | Реализован; общая сетевая приёмка остаётся открытой |
| 4. Клиенты Linux/Windows/Android и выпуск | **Текущий незавершённый этап**; beta42 установлена на телефоне, общая публикация нового входа/UI впереди |
| 5. Независимый служебный канал | Частично реализован; HTTPS Friends не заменяет независимый control ingress |
| 6. Смена полностью недоступного gateway | Сквозная приёмка и независимые входы впереди |
| 7. Сервисные события и коммерческая версия | Начата лента уведомлений; оплата/подписки не реализованы |

Сервер служебных объявлений установлен с пустой лентой. Доступ владельца приватный,
делегирование/отзыв реализованы. Сообщений пользователям не рассылали.
Android0.1.18-beta42/code42 установлен с сохранением данных. Python73,
Android147 unit/build/lint,5 native UI/notification/service tests и1 восстановление
VPN passed. Обычный запуск вне instrumentation подтверждён: ChatDeliveryService,
ConnectionService/AWG service живы, VPN в Current Networks; installed SHA256 совпал.
Локальные имена сохраняются зашифрованно и отображаются в диалогах/уведомлениях.
Серверная лента HTTP200/пустая, anonymous publish403; старый API400 на invalid input.
Публичные APK/каталоги не менялись: beta35 пока остаётся пользовательской загрузкой.
Далее: два телефона в фоне/длительный Doze/Android13+ consent/OEM/boot и общий выпуск.
[Точные версии, проверки и откат](releases/2026-09-23-messenger-notifications.ru.md).
[Настройки, администраторы и откат](testing/messenger-notices.ru.md).


## Описание GitHub обновлено — 2026-09-23

Опубликовано в main:2d98873. README RU/EN, новый native screenshot beta38,
инструкции и актуальные загрузки обновлены;232 локальные ссылки passed,
APK beta35 скачана и SHA256 сверена, desktop URLs HTTP200. Публичные README,
изображение и RU-инструкция скачаны с GitHub и совпали побайтно.
Beta38 обозначена как тестовая; публичная beta35 не заменялась.
Откат документации:revert2d98873, без изменения клиентов/серверов.
Далее: общий выпуск клиентов остаётся отдельной незавершённой задачей.
[Отчёт](releases/2026-09-23-public-client-status.ru.md).

## В работе: приглашение без ввода ключа и единый интерфейс — 2026-09-23

Доступ остаётся только по приглашению. Подготовлены обработчики ссылки в трёх
клиентах и обмен приглашения в приложении; сервер открытой регистрации не добавлен.
Android — образец внешнего вида; страна/протокол на главной, доступ/настройки в
основном окне. Python123, GTK24, локальная сборка Android/Windows прошли.
Windows54506d8: ordinary-user broker/UI и сборка прошли CI; AWG/TCP runtime
ещё выполняются. Android CI остановился на setup-android; release job skipped.
Android beta38/code38 установлен поверх прежнего с сохранением данных:147 unit,
lint/build,5 native UI tests и1 восстановление VPN (on/health ok) passed.
Селекторы показаны на месте подписей рядом с батареей; native render проверен.
Исправлены измерение селекторов и повторное измерение карты после навигации.
На экране QR убран повтор referral_hint после загрузки, подсказка остаётся одна.
Linux24 layouts и country/language/navigation passed, native render проверен.
README RU+EN и актуальный скриншот опубликованы в main2d98873;232 ссылки проверены.
Ничего не опубликовано: beta35/Linux146d/Windowsa6c68fe остаются действующими.
Далее: завершить native UI/URL handler проверки, immutable builds и только затем
одновременно обновить загрузки и страницу приглашения/CSP. Сервер/БД не менялись.
[Изменения, проверки, rollout и rollback](releases/2026-09-23-invitation-link-ui.ru.md).


## Проверка VPN, нагрузки и ошибок — 2026-09-23

Friends: сейчас4 активных устройства NL AWG, RU0/TCP0. В базе16 активировано,
12 не отозвано,4 отозвано (+3 активации со вчерашнего отчёта).
CPU в коротких замерах RU8.9–20.9%/NL1.0–9.3%, диск64.9%/18.3%.
Службы работают, HTTPS200. Сегодня при совпадающих по времени автообновлениях
ОС AWG перезапускался: RU stop timeout20s/SIGKILL с успешным запуском,
NL штатная остановка5s. Полной HTTP-статистики нет; суточную активность
по handshake после рестарта установить нельзя. Это устройства, не люди.
Далее: завершение RU AWG по SIGTERM и агрегированная история активности/ошибок;
прежние release gates/аудит отзыва/план мощности остаются открыты.
Версии по прежнему отчёту beta35/Linux146d221d0ad23c07/Windowsa6c68fe;
новых сборок, rollout/rollback нет, серверы не меняли.
[Подробный замер и ограничения](releases/2026-09-23-vpn-health.ru.md).


## Проверка VPN и клиентов — 2026-09-22

Friends: 3 активных устройства (NL AWG), RU0, TCP0; handshake ≤5 минут,
у всех3 подтверждён трафик. Всего13 активировано,9 не отозвано,4 отозвано.
Службы работают, HTTPS200, CPU RU13.8–20.7%/NL2.6–5.5%, диск65%/18%.
Это read-only снимок, не сквозной тест и не число уникальных людей.
План50 клиентов — целевая мощность, не текущее измерение.
Выпуски не менялись: beta35/Linux146d221d0ad23c07/Windowsa6c68fe
(по последнему отчёту; артефакты заново не проверялись).
Далее: прежние release gates/план мощности; при отдельном аудите проверить
старые pilots и согласованность отзыва на gateway. Rollout/rollback отсутствуют.
[Замер, ограничения и оставшиеся проверки](releases/2026-09-22-vpn-health.ru.md).

## План мощности NL для50 клиентов — 2026-09-20

Скриншот тарифов: AMS x4=200 Мбит/с, x8=300, x16=500; гигабитного варианта нет.
Ориентир для50 одновременно активных клиентов —1 Гбит/с с проверенными условиями
длительной нагрузки;4 vCPU/4 ГБ как стартовый кандидат под замеры, не гарантия.
Подтверждено50 одновременно активных клиентов. Рассматривается смена провайдера;
Последнее предпочтение —2–3 менее мощных VPS в Европе; кандидат3×500 Мбит/с,
2vCPU/4GB каждый. Доступность локаций/итоговая цена и межсерверное распределение
ещё не проверены; автоматический failover этим планом не реализован.
Миграция, оплата, backup и изменение оценки200 Мбит/с пока не выполнялись.
Текущие версии beta35/Linux146d/Windowsa6c68fe и публичные загрузки сохранены.
[Расчёт, порядок переноса, откат и оставшиеся проверки](releases/2026-09-20-nl-capacity-plan.ru.md).
...
## Глобальный приоритет после UI и мониторинга — 2026-09-20

По-прежнему этап4: доведение клиентов и выпуска. Три загрузки доступны,
Linux-навигация исправлена, мониторинг RU/NL внедрён. Установленный Linux
c55149740c24618c новее публичного11c53d18c3bc10a9; Android beta25, Windows21b740a.
Не закрыты: Windows на физическом ПК, российская сеть, длительная устойчивость,
самостоятельная установка Linux. Ёмкость серверных каналов неизвестна.
Этап5 частично реализован, сквозное восстановление этапа6 и коммерческий этап7
впереди. Без доступного Windows можно продолжать независимую доставку конфигураций
и подготовку смены gateway; масштабирование по нагрузке пока проект, не автоматика.
[Глобальный план](ROADMAP.ru.md). Это сверка документации, новых установок/тестов нет.


## Проверка нагрева ноутбука — 2026-09-20

Read-only диагностика: CPU package82→74°C; Firefox main57–59% одного ядра,
web process28–31%, терминал20–26%, GNOME19–24%, Codex8–12%.
Family Connect c55149740c24618c в отдельном5s замере0.6% одного ядра.
Причина конкретной вкладки/GPU не установлена; остановок и настроек не меняли.
При продолжении диагностики — изолировать нагрузку Firefox, затем терминала;
показания коротких замеров не доказывают единственную причину нагрева.
[Измерения и ограничения](releases/2026-09-20-laptop-heat.ru.md).
...
Основной этап — 4: завершение трёх клиентов и распространения; этапы 5–7
не закрыты. Android beta25 и Linux cce586a74c391108 установлены; Windows
21b740a прошёл платформенные jobs, физический ПК пока недоступен.
Три загрузки на странице приглашения ещё не подтверждены: workflow публикации
preview 35502270287 success, публичные байты и страница требуют проверки.
Следующее: завершить страницу загрузок, затем оставшиеся условия выпуска.
Российская сеть и длительная устойчивость остаются открытыми проверками.
[Сверка и оставшиеся действия](releases/2026-09-20-global-checkpoint.ru.md).
...
Исходники и отчёт опубликованы в desktop-ветке; docs checkpoint c20c16c.
...
## Сверка документации и CI — 2026-09-20

Read-only GitHub API подтвердил итог на source `6ed47bc`:
Client builds35474122321 — failure; native Windows TCP35474122315 и
AWG35474122305 — completed/success. Прежние записи pending ниже исторические.
Первый следующий шаг прежний: локализовать Windows `Clipped or overlapping content`,
исправить геометрию без ослабления проверки, повторить Windows CI и просмотр рендера.
Затем Linux Friends VPN apply/recovery, native AWG3.1 и /16, desktop messenger/QR/карта,
живые платформенные проверки и новая immutable поставка.

Список GitHub releases подтверждает public desktop `v0.2.9` (prerelease),
TCP `tcp-v0.1.0` и `tcp-setup-v0.1.0`. `/releases/latest` возвращает404:
для этой сверки использован список releases. Android по последней подтверждённой
установке/публикации — `0.1.18-beta19/code19`, ARM64; сегодня APK и телефон не проверялись.
Desktop worktree `/tmp/fc-desktop-friends-ci` — `e6ad985` (последние два коммита docs);
корневая папка — `main/3f6f662`, накопленные изменения сохранены.
Новых runtime tests, установок, server rollout или подписания не было.
[Аудит, ограничения документации и пути продолжения](releases/2026-09-20-vpn-documentation-audit.ru.md).
...
## GitHub: оформление опубликовано — 2026-09-19

В origin/main опубликованы 9c4114b (38 documentation/template/checker/image файлов)
и d87f30d (проверенный результат публикации). Публичный README побайтно совпал.
602 ссылки /36 Markdown,17 внешних HTTP200,4 Mermaid passed; APK, runtime и CI
не менялись. Android beta19 и desktop v0.2.9 прежние. About не изменён: gh/API
авторизация недоступна; точная команда владельцу в отчёте. Лицензия не выбрана.
[Отчёт и полный список файлов](releases/2026-09-19-github-presentation.ru.md).

Публикация сделана из /tmp/fc-github-presentation. Основная рабочая папка всё ещё
на локальном main/3f6f662 с сохранёнными незакоммиченными изменениями приложения;
origin/main уже d87f30d. Не делать слепой reset/checkout/pull поверх этих изменений.
Публичные STATUS/PLAN сокращены; прежние публичные версии сохранены в .before-2026-09-19.md.
Этот локальный журнал сохраняет подробные незакоммиченные этапы. Следующий шаг:
reviewed source checkpoint beta19 и продолжение desktop, отдельные от оформления.
Откат оформления — git revert документационных коммитов, без отката APK/ключей/DB.

Уточнение по приглашениям: пользователь подтвердил прямой код, поэтому общий
реферальный остаток500 не должен уменьшаться. Ошибки списания не обнаружено.


## Проверка счётчика приглашений — 2026-09-19

На API read-only проверены агрегаты: реферальных кодов выдано 0, активировано 0,
остаток 500. Прямые 50 — отдельный бюджет; использованная пользователем ссылка
не установлена. В Android подсказка 500 — лимит, remaining загружается однократно.
APK/сервер не менялись. [Отчёт](releases/2026-09-19-referral-counter-check.ru.md).

## Desktop: identity storage и восстановление — 2026-09-19

Реализованы Windows DPAPI+SID FriendsIdentityVault и broker public identity actions;
прежний WG-ключ сохраняется, corrupt/partial state не создаёт новую identity.
Linux получил strict Friends owner/adoption и подключение к Friends API через
from_linux_store. Legacy loaders обеих платформ не регенерируют enrolled ключи.
[Отчёт, проверки и rollback](releases/2026-09-19-desktop-identity-storage.ru.md).

Python53 scoped /549 full passed (2 upstream warnings). .NET protocol passed,
Windows app/BrokerTests cross-build0errors/0warnings. Windows control CI35469710256
success на429eb77; Windows Client builds35469710250 install/broker/restart/UI checks
success; layout и Windows job completed/success. Linux client CI success.
Android setup-android failed до сборки, как и на предшествующем0595f884; весь run
не зелёный. Установщиков для пользователя и release/catalog не выпущено.
Ветка desktop/friends-access-20260919/429eb77 отдельная; main/Android beta19 неизменны,
desktop public по-прежнему v0.2.9. Тестовые CI assets с0.2.9 не публиковать поверх релиза.

По уточнению пользователя Android beta19 оставляем: APK не пересобираем и не
переустанавливаем, ссылку скачивания не меняем. Android CI запустился автоматически;
исправление его SDK setup сейчас не входит в desktop-этап.

Далее: signed Friends profiles/sequence floor, Windows AWG3.1 compatibility,
apply/journal/ownership, desktop UI и messenger. Пользователь готов проверить Windows.
Новая выдача identity при потере данных — только отдельная явная регистрация после
решения о recovery, не удаление marker/профилей. Продуктовое оформление GitHub локально;
коммерческая модель и лицензия не выбраны.


## GitHub и обновление desktop — 2026-09-19

Подготовлено локальное продуктовое оформление README EN/RU, актуальные инструкции,
карта документации, Security/privacy, contribution templates и лицензионный аудит.
Проверены 216 ссылок в 17 публичных входных документах (локальные пути/anchors/images),
четыре внешних URL вернули HTTP200, четыре Mermaid-диаграммы отрисованы Chromium
с Mermaid10.9.3. `git diff --check` passed. Runtime-тесты не повторялись для docs.
[Подробный аудит, изменённые файлы и ограничения](releases/2026-09-19-github-presentation.ru.md).
Commit/push/About не выполнялись; прежние незакоммиченные изменения сохранены.

Владелец уточнил коммерческую цель: заработок на VPN-сервисе. Лицензия НЕ выбрана,
MIT не разрешена; модель оплаты и лицензирования требует решения. Скачивание APK
не равно серверному доступу. Сторонние GIF имеют неустановленные условия распространения.

Новый приоритет пользователя: привести Linux и Windows к актуальному состоянию.
Публичные desktop пока v0.2.9; Android — 0.1.18-beta19/code19. Аудит разницы завершён; [план desktop](desktop-modernization.ru.md).
Реализован первый Friends API/identity блок (без UI/DPAPI/VPN apply): 25 новых
Python checks, полный Python537 passed/2warnings; .NET protocol executable passed.
[Отчёт](releases/2026-09-19-desktop-friends-foundation.ru.md). Отдельная Windows-ветка
desktop/friends-access-20260919/0595f884 отправлена в CI; Windows run35468900333
**completed/success**: protocol conformance на Windows passed.
Это не DPAPI/UI/живой VPN приёмка. Новых desktop установок, releases и каталогов нет. Linux GTK/libadwaita сохраняется;
Windows runtime/UI проверять Windows CI, не считать Linux cross-build приёмкой.
Пользователь подтвердил доступность компьютера Windows. Далее: protected identity
и journal/broker integration, signed Friends profiles, UI/мессенджер, platform checks.
Новые immutable releases только после platform checks, проверки artifacts и offline signing;
откат — штатный previous/новая версия, без удаления device state и снижения floors.


## Смайлики: текущий набор оставлен, переработка отложена — 2026-09-19

Пользователь подтвердил на beta16: входящий смайлик появляется без набора
ответа и двигается. На beta17 исправлялась белая кайма/верхнее обрезание,
но пользователь сообщил о бахроме. Beta18/19 добавили мягкую прозрачность,
меньший масштаб и удалили заголовок панели «Смайлики ICQ» (доступная подпись
«Смайлики» сохранена). Пользователь снова отклонил контур beta19: выглядит грязно.
Пользователь посмотрел пять новых векторных образцов и отклонил этот стиль:
«нет. оставляем эти пока что». Текущие GIF в приложении остаются;
перерисовка и интеграция нового набора отложены. Это не подтверждение исправления
бахромы. [Отклонённый макет](design/smileys-smooth/index.html) сохранён как
дизайн-эксперимент; [отчёт](releases/2026-09-19-smooth-smileys-preview.ru.md).

Текущая установленная Redmi Note9 Pro/Android12 —0.1.18-beta19/code19,
ARM64/36,4МБ, прежний beta-v1 ключ.140 unit +2 native UI passed,
lint0errors/16warnings, GIF clock/pixels/pause/resume/line metrics проверены.
SHA2565ebe38168f084d3e19bb740722ec7c3a7ceb5e9ed63508efc3e3ff3f5e0bf3a4
совпадает с установленной APK. Тесты не заменяют отклонённую визуальную оценку.

После замечания пользователя о скачивании beta16 ссылка /invite/ переключена
на существующую beta19 (0.1.18-beta19/code19, 36 390 988 байт), без пересборки.
Проверка nginx -t пройдена, конфигурация перечитана. Полное HTTPS-скачивание
совпало по SHA256, страница побайтно совпала с локальным HTML; [отчёт публикации](releases/2026-09-19-beta19-download.ru.md).
Beta17/18 не публиковались; старые публичные APK сохранены.
[Принятая анимация и входящие beta16](releases/2026-09-19-android-beta16.ru.md) ·
[Непринятый контур beta19, проверки/rollback](releases/2026-09-19-android-beta19.ru.md).
Предыдущие APK неизменны; возврат клиента — новая версия code>19 с тем же ключом,
без удаления приложения/истории. Сервер доставки/инвайты не менялись.
Переработку смайликов не продолжать без нового запроса. Остаются проверки мессенджера:
другие устройства, offline/restart/дубли; затем запланированное продуктовое оформление GitHub.


## Friends APK 0.1.14-beta15: новый экран мессенджера и ICQ — 2026-09-19

Пользователь подтвердил обмен сообщениями между телефонами на beta14. По его
следующему запросу мессенджер переработан: список диалогов/последнее сообщение,
отдельная переписка, входящие слева/исходящие справа, постоянный ввод и отправка,
49 ICQ GIF и кнопка выбора. Служебные действия спрятаны в меню; нижние вкладки
сохранены. Обновление истории не пересоздаёт composer/курсор; черновики между
диалогами в RAM (не после смерти процесса). Контакты пока с короткими ID.

Текущая установленная версия Redmi Note9 Pro/Android12 — beta15/code15,
ARM64/36,4МБ, прежний beta-v1 ключ. 136 unit,2 native UI passed; lint0errors/16warnings;
49 GIF/69 кодов проверены, native/payload/signature/zipalign passed. UI-тесты
проверяют синтетическую переписку, GIF spans, draft/cursor, отправку исходного
текста и composer в resized viewport. Рендер просмотрен. Пользовательские
контакты/сообщения не изменялись тестами. Новая живая переписка на beta15 ещё
не подтверждена; прежний обмен beta14 не заменяет эту проверку.

[Скачать beta15 ARM64](https://185.251.89.19:8443/downloads/FamilyConnect-Test-0.1.14-beta15.apk).
QR/ссылка из приложения теперь предлагает beta15. APK полностью скачана по HTTPS,
SHA256 094b19489ad8ebd53e30a5fca2c69078d7ce5621699ad2aa169c939bd854d4ec
совпадает с локальной и установленной APK. Nginx -t/HUP и HTML проверены.
Обновление на других телефонах — установить APK поверх, без удаления приложения
и без повторного инвайта. Автообновление не реализовано. Серверы доставки и пул
приглашений не менялись; GitHub release/подписанный каталог не создавались.

[Проверки, артефакт и rollback](releases/2026-09-19-android-beta15.ru.md).
Beta14/13 сохранены неизменными; откат страницы — before-beta15-download backups,
возврат UI — новая версия с code>15 и тем же ключом, без удаления данных.
Далее: пользовательская оценка интерфейса/смайликов, переписка beta15 на двух
телефонах, offline/restart/дубли, другие устройства/крупный шрифт/клавиатуры.
Затем остаётся обязательное продуктовое оформление GitHub.


## Friends APK 0.1.13-beta14: компактная загрузка по QR — 2026-09-19

Текущая установленная версия на Redmi Note9 Pro/Android12 — beta14/code14,
ARM64,36,4МБ вместо универсальной beta13/240,5МБ (уменьшение примерно85%).
Оставлена одна архитектура и включено сжатие библиотек; все9 native .so
побайтово совпадают с beta13. По умолчанию CI/Gradle сохраняет четыре ABI;
компактная сборка включается явно -PfcTargetAbi=arm64-v8a.

136 unit,2 native UI (включая загрузку библиотеки) passed; lint0errors/10warnings,
payload/signature/zipalign passed. Первый UI запуск не вывел главный экран;
после открытия/разблокировки пользователем повтор passed. Рендеры просмотрены.
SHA256 d0da327b2bd8c6a8806bb8f5debf07c43d45bfe8cb0aea1c363bb307d69cc13c
совпал у локальной, установленной и полностью скачанной публичной APK.

[Скачать beta14 ARM64](https://185.251.89.19:8443/downloads/FamilyConnect-Test-0.1.13-beta14.apk).
QR/ссылка из приложения ведёт на /invite/: получение личного кода и скачивание
на одной странице. Код пока вставляется вручную; пользователь подтвердил,
что достаточно QR и ссылки со скачиванием. Для остальных архитектур там же
оставлена неизменная универсальная beta13. Новая APK установлена поверх без
удаления данных; ключ прежний. Nginx -t/HUP и публичный HTML проверены.
VPN/DB/пул инвайтов не менялись. GitHub release/каталог обновлений не создавались.

[Артефакт, проверки и rollback](releases/2026-09-19-android-beta14.ru.md).
Откат страницы — backups before-beta14-download; возврат клиента — code>14
с тем же ключом, без удаления данных и без перезаписи артефактов.
Пользователь подтвердил «подключился» на компактной beta14 (2026-09-19).
Это подтверждение подключения на его телефоне, не отдельный замер трафика
или проверка всех серверов/транспортов. Далее: сообщения/QR на двух телефонах,
другие ARM64 устройства и полная VPN-матрица. Затем обязательное продуктовое
оформление GitHub.


## Friends APK 0.1.12-beta13: единые рамки и точечный маршрут — 2026-09-19

Текущая установленная версия — beta13/code13 на Redmi Note9 Pro/Android12.
После beta12 сохранены общий знак двери, постоянные вкладки и приблизительная
геопозиция Android. Убраны наложенные прямые/скруглённые контуры и кнопки возврата;
шапка получила две тонкие короткие линии. Маршрут — бегущие точки от устройства
к региону VPN. Запрос положения теперь использует доступные fused/network источники
и явный getCurrentLocation на Android30+, отменяет предыдущую попытку.
Исходная причина сообщения «появилось после смены сервера» точно не установлена.
В native проверке beta13 положение и пунктир появились без смены сервера.

136 unit +2 native UI passed, lint0errors/14warnings; payload/signature/zipalign passed.
APK `state-client-build/android-pilots/beta13/FamilyConnect-Test-0.1.12-beta13.apk`.
SHA256 `b6144bd3e131a2f43d628f992f3df3d56990127ae192d4e5b65132f389aaea38` совпадает с установленным файлом.
Рендеры просмотрены. Предыдущая beta12 сохранена неизменной; возврат UI — новой
сборкой с code>13 и прежним beta-v1 ключом, без удаления данных.
[Версия, проверки, rollout/rollback](releases/2026-09-19-android-beta13.ru.md).

Пользователь подтвердил «все работает» после проверки beta13: текущие изменения
интерфейса и маршрута приняты на его телефоне (2026-09-19).
Пользователь также подтвердил работу VPN на своём телефоне; сервер/транспорт
и полная матрица этим не подтверждены. Начинается установка на другие телефоны
для проверки сообщений. Публичная /invite/ доступна по HTTPS, но ещё содержит
старую заметку о подготовке APK и не предлагает скачать сборку: beta13 пока
передаётся отдельным файлом. Выдача кода при проверке страницы не вызывалась.
Далее: обновить путь установки на странице приглашения, проверить messenger/QR
на двух телефонах; TalkBack/крупный шрифт/landscape/Android15 и VPN-матрица.
Затем обязательное продуктовое оформление GitHub. Публичной публикации нет;
серверы/инвайты не менялись. Ранее проверенные 50/500 не являются новым подсчётом.


## Friends APK 0.1.10-beta11: телеметрия и полноэкранный маршрут — 2026-09-19

Главный экран без прокрутки: SYS-строка убрана, версия перенесена в настройки,
большие дублирующие кнопки убраны, переключатель51×31dp (зона52dp). Активация —
через «Подключить»/настройки; «Чаты» переименованы в «Мессенджер». Добавлены
реальные тип сети, доступный Wi-Fi RSSI, батарея и график трафика приложения.
Маршрут — полноэкранная офлайн-карта Natural Earth, pinch/pan, просмотр регионов
по нажатию, состояние туннеля/проверки сети и выбор RU/NL × AWG/TCP.
Путь логический, положение устройства условное; промежуточные узлы не выдумываются.

Friends0.1.10-beta11/code11 подписана прежним beta-v1 ключом и установлена поверх
на Redmi Note9 Pro/Android12. APK: `state-client-build/android-pilots/beta11/FamilyConnect-Test-0.1.10-beta11.apk`.
SHA256 `451b558a68090f50c74e2a56201633b1781dc8538f8345c099f14ca4693aa245` совпадает с установленным файлом.
136 unit +2 UI-теста passed, lint0errors/14warnings; native/payload/map,
signature/zipalign passed. UI-тесты проверили viewport320×512dp, полный экран
маршрута, нажатие NL, настройки и сохранение страны/транспорта. Рендеры просмотрены.

Следующее: пользовательская оценка/ручные жесты, TalkBack, крупный шрифт,
landscape/Android15; активация, физическая VPN-матрица, два телефона для чата/QR.
После messenger/device acceptance остаётся обязательное продуктовое оформление
GitHub и публичная раздача. Серверы и инвайты на этом UI-этапе не менялись;
50/500 — прежний результат проверки, не новый подсчёт. Публичной публикации нет.
Beta10 сохранена; возврат прежнего UI — новой сборкой с code>11 и тем же ключом.
[Артефакт, проверки и rollout/rollback](releases/2026-09-19-android-beta11.ru.md).

## Friends APK 0.1.9-beta10: главный экран без прокрутки — 2026-09-19

По запросу пользователя переработан Android UI: главный экран без ScrollView,
адаптивный круговой индикатор, нижние четыре раздела; выбор страны/транспорта и
проверка IP перенесены в «Маршрут», анимация — в «Настройки». Добавлены оранжевые
акценты и переключатели с оранжевым выключенным / зелёным включённым состоянием.
Анимация следует состоянию сервиса, учитывает видимость и может быть отключена.
Сопоставлены три пользовательских изображения и авторские материалы Silo/Territory.

Friends0.1.9-beta10/code10 подписана прежним beta-v1 сертификатом и установлена
поверх приложения на Redmi Note9 Pro/Android12; данные сохранены.
APK: `state-client-build/android-pilots/beta10/FamilyConnect-Test-0.1.9-beta10.apk`,
SHA256 `6122f4bc23e54f34978d7032d49648129bb4585457876c6825f47f55474404b9`.
136 unit и2 физических UI-теста passed; lint0errors/13warnings; payload/signature/
zipalign passed. SHA256 установленного APK совпадает. Просмотрены native рендеры
основного экрана, рабочей области320×512dp, переключателей и настроек.

Следующее: пользовательская оценка оформления, ручная активация и RU/NL × AWG/TCP,
два телефона для чата/QR; отдельно landscape/Android15/крупный системный шрифт.
Потом остаётся обязательный этап продуктового оформления GitHub и публичной раздачи.
Серверы и инвайты в этом UI-этапе не менялись; прежние50/500 — результат предыдущей
проверки, а не новый подсчёт. Публичный release/download ещё не опубликован.
Beta09 сохранена; возврат прежнего UI — новым code>10 с тем же ключом, без удаления
данных. [Сборка, UI-приёмка и rollback](releases/2026-09-19-android-beta10.ru.md).

## Friends APK 0.1.8-beta09 собрана и установлена — 2026-09-19

Пользователь явно снял APK-паузу. Универсальная Friends0.1.8-beta09/code9
(`com.familyconnect.app.friends`) собрана из текущих исходников, подписана постоянным
локальным beta-v1 key и установлена на Redmi Note9 Pro/Android12. Главный экран
запускается; SHA256 установленного APK совпадает с подписанным файлом.
...
Четыре ABI; chat/carrier/LXMF/QR и лицензии присутствуют. JVM108/Friends unit136
passed, lint0errors/11warnings, payload/signature/zipalign passed. На физическом
телефоне прошли4 chatchecks и2 проверки prepare/force-stop/resume.

MIUI запрещает ADB input (`INJECT_EVENTS`); автоматическая активация не выполнена.
Пользователю предложена ручная активация первым кодом из пакета. Последняя серверная
проверка: первый код НЕ активирован, свободны все50 прямых и500 реферальных.
Прежнее намерение отметить первый код занятым ещё не стало фактом.

Следующее: ручная активация, четыре сочетания RU/NL × AWG/TCP, два телефона/QR
камерой и доставка в реальном UI; затем публичная раздача и оформление GitHub.
APK доступен локально и установлен, но публичный release/download ещё не опубликован.
[Сборка, доказательства и оставшиеся проверки](releases/2026-09-19-android-beta09.ru.md).

## 50 прямых приглашений и 500 реферальных — 2026-09-19

По новому указанию пользователя подготовлены 50 прямых одноразовых кодов
(исходные 10 сохранены + 40 новых), отдельно включён общий серверный пул на 500.
Активированный участник получает постоянную ссылку для QR; посетитель по явному
нажатию получает собственный код и после активации тоже может приглашать.
Повтор в том же браузере возвращает прежний код; 20 новых выдач за скользящие
24 часа на приглашающего. Отзыв приглашающего запрещает новые выдачи.

Развёрнуто на двух разрешённых узлах; live HTTPS цепочка, повтор и отзыв проверены.
Тестовые приглашения отозваны, только синтетическая claim-запись удалена из счётчика:
остаются 50 неиспользованных прямых кодов и все 500 реферальных мест.
Приватный файл: `state-enroll/friends-pilot/FamilyConnect-invitations-50.txt`,
mode0600, игнорируется Git. Коды и campaign key не публикуются.

Android source: экран «Пригласить друга», QR и копирование ссылки в терминальном
стиле. Java compile и QR decode прошли; APK НЕ собирался и не устанавливался.
512 общих тестов/2 прежних warnings, 114 messenger, 13 focused passed.
Membership допускает 550 динамических identity + 2 диагностические; это лимит
допуска, а не подтверждение производительности на 550 устройствах.

Следующее: native runtime/сканирование двумя телефонами и release gates после
снятия APK-паузы; затем продуктовое оформление GitHub и массовая раздача.
Страница приглашения пока честно сообщает об ожидании Android-сборки.
[Развёртывание, rollback и ограничения](releases/2026-09-19-referrals.ru.md).

## Рабочая регистрация и доставка чата — 2026-09-19

Развёрнуты friends chat API/registry на185.251.89.19 и expiring membership на
Amsterdam186.246.45.246. Timer20с, lease100с, отдельный forced-command SSH key,
проверка участников на каждом запросе, сохранены2 диагностические identity.
Восстановлен обнаруженный failed/start-limit-hit mailbox; сейчас active/success,
NRestarts0, том1%. Инициатор остановок14.09 не установлен.

Live: два временных пользователя прошли HTTPS activation/регистрацию, обменялись
сообщением/ответом; после отзыва тот же клиент получил access_denied. Оба тестовых
invite/device отозваны. Подготовленные10 не расходованы; цель50 сохраняется.
Android source включает доставку после active/bootstrap и обрабатывает network
lifecycle; native runtime ещё не принят. Ручное обновление входящих, без Doze/push.

Проверки:507 общих/2 warnings,114 messenger, Java compile passed; public source
SHA256 сверены с обоими серверами. APK, push, release/signing не выполнялись.
Backups на обоих хостах: соответствующий service root/backups/20260919-141024/.
[Полный rollout/rollback и ограничения](releases/2026-09-19-chat-live.ru.md).

Следующее: APK/runtime и два телефона после снятия пользовательской паузы; затем
оставшиеся release gates, продуктовое оформление GitHub и раздача50 инвайтов.
По вопросу пользователя: новые VPN-регионы пока добавляются оператором с изменением
Friends ru/nl API/клиента и подписанного каталога; автоматического подключения VPS нет.

## Регистрация чата по активированному приглашению — 2026-09-19

Подготовлены ChatAccess и Android регистрационный путь: challenge связывает
активированное устройство и конкретный chat key, нужны подписи обеих identity;
replay/expiry/revocation/подмена ключа отвергаются. Один неизменяемый chat key на
устройство. На сервере additive таблицы создаются только явной миграцией; она
на рабочих узлах ещё не выполнена. Ответ pending-node не включает доставку.
Android показывает статус ожидания и сохраняет локальную очередь. Allowlist8 modules.

Проверки:505 общих passed/2 warnings;25 focused bridge/friends passed; Java source
compile passed без APK (точный classpath и полный messenger result в отчёте).
Никаких новых инвайтов/серверных изменений/публикаций. APK-пауза сохраняется.

До раздачи: применить регистрацию/membership/отзыв на узле и доверенный bootstrap,
подключить carrier к UI; затем после снятия паузы собрать и проверить два телефона,
закрыть release gates и оформить публичный GitHub. Цель50, подготовлено10.
Следующая работа — этот сетевой путь, без расширения косметического этапа.
[Checkpoint, проверки, rollout/rollback](releases/2026-09-19-chat-admission.ru.md).

## Native экраны локального чата — 2026-09-19

Подготовлена ChatActivity в утверждённом терминальном стиле; вход «Чаты» добавлен
в MainActivity и FriendsActivity. Есть явное создание локальной identity, resume,
контакты, своя публичная карточка/копирование ключа, проверка полного fingerprint
перед сохранением контакта, переписка с50 сообщениями на страницу и durable queue.
Открытие переписки показывает последнюю страницу. Статусы queued/sending/relayed/
delivered/received различаются; сейчас UI честно сообщает, что доставка не подключена.

Один FIFO worker между Activity instances закрывает старого owner до открытия
нового; main thread не выполняет операции Store/Keystore/Python. Поздние UI replies
отсекаются по lifecycle generation, повторное нажатие заблокировано на время commit.
Ошибка записи сохраняет текст; успешный commit очищает его, включая возврат/rotation.
Draft переживает rotation только в RAM, не process death. FLAG_SECURE, no-autofill,
отсутствие plaintext draft в saved-state; повреждение/остатки state не ведут к create.

Проверки этого шага: javac17 восьми связанных Java classes против Android35 и
Chaquopy16.1.0 (временный R из XML, compile classpath Gson2.10; app pin2.13.2);
28 Python bridge/local tests passed, EN/RU resources/manifest/new doc links и diff
проверены. Это не AAPT/Gradle/lint/Android UI runtime; APK не собирался.
Предыдущие полные наборы104 messenger/497 общих остаются последней полной проверкой.

Дальше: доверенная регистрация реальных chat identities/mailbox bootstrap и привязка
сетевого lifecycle к UI, затем QR/смайлы/доработки удобства; runtime/два телефона после
снятия паузы APK. Публичный GitHub и50 инвайтов остаются после мессенджера.
[Полный checkpoint и rollout/rollback](releases/2026-09-19-android-chat-ui.ru.md).

## Общая картина и Android carrier — 2026-09-19

**Стадия продукта: интеграция перед закрытым пилотом, не публичная готовность.**

| Блок | Текущее состояние | До раздачи |
| --- | --- | --- |
| VPN | Транспорты/переключение реализованы, есть CI и ограниченные live-проверки | Оставшиеся platform/release gates и физическая приёмка |
| Ядро чата | История, контакты, очередь и mailbox exchange проверены | Интеграция с платформой |
| Android chat | Java/Python owner и bounded carrier подготовлены; Python wire acceptance прошёл | Реальный enrollment, native экраны и runtime |
| Внешний вид | Терминальный стиль одобрен, HTML-макет и оформление существующих Activity подготовлены | Реализовать native chat screens и проверить на устройстве |
| Два телефона | Ещё не принято | Offline/restart/Wi-Fi/mobile/сон/VPN |
| Публичный GitHub | Требования сохранены | Оформить после мессенджера |
| Раздача | Цель50 инвайтов, подготовлено10 | Проверенный подписанный выпуск, затем оставшиеся40 |

Новый fc_chat_transport.CarrierMailbox использует общий RNS singleton/session lock
с control channel. Java ChatNetworkAndroid привязывает каждый socket к underlying
non-VPN network до connect. Соединение создаётся на один обмен и удаляется после
него; переподключение выполняет DeliveryController, RNS не создаёт unbound socket.
Busy от занятого control channel получил ограниченные повторы с backoff.
LocalChat/Session/ChatLocalAndroid получили configure_delivery и lifecycle methods;
bootstrap не принимается через JSON UI API. Сеанс закрывает worker до destination.
Mailbox-only destination без постоянного LXMRouter job loop и direct receive.

Это ещё не подключённые Activity lifecycle/enrollment и не принятый Android runtime.
Native binding проверен только компиляцией отдельного callback; Python carrier —
реальным loopback exchange. Полный messenger suite104 passed, общий suite497 passed
с2 dependency warnings; diff/link checks passed. Новый instrumentation сценарий подготовлен, не запущен.
APK-пауза, отсутствие push/release/server changes сохраняются. Версии прежние:
Android0.1.7-beta08/code8, исходник3f6f662 + локальные изменения.

Дальше: native chat UI/owner lifecycle и доверенная регистрация пользовательских
chat identities; после снятия паузы — APK/runtime и приёмка двух телефонов.
Полные проверки и rollout/rollback: [Android carrier checkpoint](releases/2026-09-19-android-chat-carrier.ru.md).

## Сейчас: очередь доставки мессенджера, APK на паузе — 2026-09-19

Пользователь попросил пока не собирать APK и заняться внешним видом. Переданы
три PNG-референса, затем уточнён ориентир: программное обеспечение из сериала
«Бункер» (Silo). До снятия паузы не запускать assemble/package/emulator APK jobs
или push, который автоматически запустит Client builds. Source/JVM/Python/HTML
проверки допустимы. APK после этого указания не собирался и не устанавливался.

Подготовлены TerminalUi и оформление существующих MainActivity/FriendsActivity:
тёмно-зелёный фон, тонкие рамки, моноширинные подписи, статический индикатор VPN
из реального service status/health, адаптация ширины. Без выдуманных процентов,
многошагового маршрута или недоступных защитных функций из референсов.
[Интерактивный HTML-макет](design/terminal-preview.html) отдельно показывает четыре
раздела; вкладки чата/настроек в нём — дизайн-направление, не готовые native screens.

Перед паузой подготовлен Android local bridge ChatLocalAndroid/ChatStoreCipher,
Python Session, общий PythonRuntimeAndroid, allowlist упаковки7 core modules и
изолированный instrumentation harness. Ключ Store остаётся в Java; Python получает
AES-GCM callbacks. Native bridge и Keystore/process-death ещё не приняты в runtime.
Новый messenger Android CI job выключен по умолчанию, только manual opt-in;
это не отменяет общую паузу сборок. Последняя сверенная CI/release версия прежняя.

Проверки после delivery:95 messenger tests passed,497 общих tests passed/2 dependency warnings;
Java/Python AES-GCM fixture passed; TerminalUi javac с Android35 passed;
Gradle tasks configuration passed. Попытка assemble до паузы остановилась на
отсутствующих SDK components/licenses, готового APK нет. HTML просмотрен в Firefox
430/360px;19 browser interaction/overflow checks passed на320px. Это не Android
rendering/interaction acceptance. Версии продукта не менялись.

Пользователь одобрил показанный интерфейс и поручил продолжать. Добавлены
Mailbox.exchange и DeliveryController: получение до10 сообщений и отправка до4
на одном link/deadline, ограниченные повторы временных ошибок, без idle polling.
LocalChat будит worker после commit очереди и сохраняет владение Store/ключом,
пока close не дождётся остановки. Это подготовленное Python-ядро, не включённый
Android carrier. Статус relayed не означает доставку собеседнику.

Следующее: подключить единого Android carrier owner и lifecycle, регистрацию
реальных chat identities и native экраны чата. APK/runtime приёмку возобновлять
только после снятия паузы. [Проверки доставки и границы этапа](releases/2026-09-19-messenger-delivery.ru.md).
Публичный GitHub — после мессенджера, целевой тираж инвайтов50 сохраняется.
[Полный checkpoint и rollout/rollback](releases/2026-09-19-android-bridge-terminal.ru.md).

## Текущий приоритет: мессенджер по этапам — 2026-09-19

Пользователь явно выбрал сначала мессенджер и поручил самостоятельно двигаться
по пунктам. Это заменяет порядок от14.09 «сначала весь VPN». Последнее сообщение
об установке VPN пользователь отменил как отправленное по ошибке.
Раздача после мессенджера: целевой тираж50 инвайтов, сейчас подготовлено10;
дополнительные40 не созданы. Небольшое обновление внешнего вида включить в этап UI.

Первый подпункт выполнен: локальный API messenger.local.LocalChat для будущего
Android owner. Явные create/resume, проверка fingerprint контакта, durable queue,
постраничная история без private keys/packed payload/attempt tokens, сериализация
close и локальных операций. Потеря базы/identity или неверный ключ при resume
не создают новую учётную запись. Исправлен порядок истории при смене статуса.
Это Python API, ещё не подключённый Java/Chaquopy/Keystore bridge и не экран чата.

Проверки: полный набор мессенджера66 passed; после добавления сценариев аварийного
выхода процесса и отказа записи все16 local tests passed (14 пересекаются с66).
Стандартный python -m pytest -q:497 passed,2 dependency deprecation warnings.
RNS1.5.1/LXMF1.1.1/cryptography46.0.7, Python3.14; pins control/provisioning/identity
согласованы, LXMF hash checked. Источник3f6f662 + локальные изменения;
новых CI, APK, установки или изменения серверов не было.

Следующие пункты:
1. Android bridge: связать этот API с ChatKeyVault, выбрать проверяемую упаковку
   AES-GCM/LXMF, единого владельца Python/RNS и проверить Keystore/process death.
2. Carrier и отправка outbox: восстановление связи, подтверждения сохранения,
   безопасная остановка, совместная работа с VPN и существующим control channel.
3. Android UI: список чатов, личная переписка, адрес/QR и проверка отпечатка,
   статусы, существующие49 классических смайлов; без групп/звонков/вложений.
4. Приёмка двух телефонов: offline, перезапуск, Wi-Fi/mobile, сон и включённый VPN;
   явно определить ограничения фоновой доставки до публикации.
5. После реализации и приёмки мессенджера выполнить обязательное
   [продуктовое оформление публичного GitHub](github-product-presentation.ru.md):
   аудит, README EN/RU, security/docs map, реальные CTA, About и проверка ссылок.
   Пользователь сообщил, что репозиторий уже открыт; оформление пока не начато.
6. Закрыть оставшиеся VPN release gates, выпустить проверенную сборку и50 инвайтов.

[Подробный результат, API и rollout/rollback](releases/2026-09-19-messenger-local.ru.md).
...
Уточнено позднее19.09: пользователь выбрал мессенджер первым, сообщение об
установке VPN отменил. Действующий порядок — в начале этого документа.
