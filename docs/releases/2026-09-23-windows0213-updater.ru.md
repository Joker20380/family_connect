...
На момент публикации проверка плавности на ПК оставалась открытой; последующая приёмка записана ниже.

Publisher [35923408333](https://github.com/Joker20380/family_connect/actions/runs/35923408333) — success.

Публичный канал проверен после публикации через production-код `Updates.Check` в C# harness:
получение `main/updates/windows.json`, проверка embedded anchor и подписи, offer0.2.13
при сравнении с0.2.12, отсутствие повторного offer для0.2.13. Sequence9 принят, floor
сохранён в изолированном тестовом каталоге. Это проверка кода0.2.13; старое приложение0.2.12
не переключается на новый канал без согласованной ручной установки.

RU/EN документация проверена:285 Markdown-файлов,1666 ссылок, ошибок нет.

## Пользовательская приёмка и закрытие этапа4 —23.09.2026

После согласования конечных критериев пользователь ответил «считай подтвердили».
По этому подтверждению приняты Windows0.2.13 (интерфейс, приглашение, подключение и
проверка обновлений) и Linux0.2.10 (установка → приглашение → подключение → отключение).
Android beta50 опирается на ранее подтверждённые сценарии. Этап4 закрыт в объёме
пилотного выпуска; новые инструментальные проверки не выполнялись, подробные результаты
с пользовательских ПК не предоставлены. Версии, артефакты и пути отката не менялись.

Следующий рабочий этап —5, независимый служебный канал. Длительная сетевая приёмка,
точная задержка Doze, desktop messenger и коммерческая готовность остаются отдельными
задачами и не удерживают завершённый пилотный этап4 открытым.
