...
Публикация в Git разрешена пользователем. Результат push проверяется отдельно;
состояние публикации следует смотреть в текущем STATUS, а не в исторических отчётах.
