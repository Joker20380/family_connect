...
Публичный `Updates.Check` выполнен после публикации main: для0.2.13
предложена0.2.14, для0.2.14 повторного предложения нет; проверены подпись и
монотонный floor10 в изолированном каталоге. Подтверждена работоспособность
публичного Windows-канала, не установка на пользовательском ПК.
