...
## Продолжение 26.09: вкладка «Статистика» вместо «Маршрут» (Linux)

После приёмки серверного списка на Linux пользователь решил заменить вкладку
«Маршрут» на «Статистика»: трафик сессии, суммарные байты/длительность и нагрузка
шлюзов NL/RU. Профиль и «Проверить IP» перенесены на «Статус». Это только правка
исходников Linux GTK; версия `0.2.11` и опубликованные каталоги не менялись.
Детали и проверки — [отчёт](2026-09-26-linux-stats-tab.ru.md).
