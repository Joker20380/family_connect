...
**Сейчас: этап 3 — короткая приёмка TCP на ноутбуке пройдена; впереди AWG → TCP recovery.**
...
| 3. TCP VLESS+REALITY | **Изолированная и короткая host-проверки пройдены; впереди recovery** | Понятны оставшиеся тайм-ауты; исправленный helper проходит короткий host-тест, точную очистку и established AWG → TCP recovery |
...
- TCP: сервер развёрнут, исправленный host helper установлен, но служба выключена;
  исправленный helper установлен и прошёл короткий host smoke, recovery ещё не принят.
