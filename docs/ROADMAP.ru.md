...
**Сейчас: этап 3 — реальный GTK/pkexec recovery прошёл; остаётся ручная отмена системного запроса прав.**
...
| 3. TCP VLESS+REALITY | **GTK recovery/close пройдены; остаётся polkit Cancel** | Понятны оставшиеся тайм-ауты; исправленный helper проходит короткий host-тест, точную очистку и established AWG → TCP recovery |
...
3. AWG → TCP backend/policy восстановился за 45.32/45.57 с; все попытки очищены.
   ...
   Close-while-connected проверен; осталась реальная отмена авторизации. [GTK-отчёт](releases/2026-09-11-tcp-gtk.ru.md).
