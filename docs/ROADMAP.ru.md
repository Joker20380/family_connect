...
| 4. Платформы и стабильный выпуск | **Desktop0.2.9 и TCP Setup0.1.0 опубликованы; Windows TCP engine + защищённый профиль проверены, lifecycle/session проверены, packaging/UI проверены; recovery/AWG и Android ещё впереди** | Linux/Windows/Android: native интеграция, установка, переключение, отмена, восстановление; platform CI, UI/runtime, подписанный неизменяемый релиз |
...
4. По подтверждённому порядку: завершить Windows (TCP health-monitor, full-routing acceptance и AWG), затем Android transports.
