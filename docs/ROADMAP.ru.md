...
**Сейчас: этап 3 — переход от изолированной проверки к короткой приёмке TCP на ноутбуке.**
...
| 3. TCP VLESS+REALITY | **Изолированная проверка пройдена; впереди host-приёмка** | Понятны оставшиеся тайм-ауты; исправленный helper проходит короткий host-тест, точную очистку и established AWG → TCP recovery |
...
2. Сделать backup и установить исправленный helper для короткой проверки на ноутбуке:
   маршруты, DNS/HTTPS, сохранность LAN/Docker и точная очистка после остановки.
3. Проверить восстановление уже установленного соединения AWG → TCP и возможность отката.
