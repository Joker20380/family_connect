...
4. По подтверждённому порядку: завершить Windows (TCP recovery, full-routing acceptance и AWG), затем Android transports.
