...
4. По подтверждённому порядку: Windows transport implementation завершена и принята в scoped CI; Android WG/AWG/TCP принят в API35 эмуляторе (18 REALITY HTTP, 3 OS DNS, 36 UDP, остановки, отмена, системный отзыв); далее Android Auto. Full-routing/device acceptance остаётся условием выпуска, отложенные тесты не возобновлять автоматически.
