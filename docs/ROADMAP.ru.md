...
Текущий основной блок — завершение этапа 4: платформы, реальная приёмка и выпуск.
Android 0.1.18-beta25 установлен и проверен на телефоне; Linux 0.2.9 preview
(bundle 11c53d18c3bc10a9) установлен с новой картой и интерактивным кругом.
Windows 21b740a прошёл UI и installer jobs; физическая проверка отложена:
компьютер сейчас недоступен. Российская сеть также недоступна.
Linux Friends RU/NL × AWG3.1/TCP прошёл короткую проверку 4/4; исправление
повторной авторизации принято отдельным живым тестом одного auth-сеанса.
Карта и круг перенесены на три платформы, Linux QR включён в установленный bundle.
Художественная доработка анимации остаётся отдельной задачей.

Страница загрузок обновлена: Android beta25, Linux paired11c53d18c3bc10a9
(source d93611e) и Windows preview21b740a. Linux приглашения встроены в общий
интерфейс с постоянными верхними/нижними элементами. Новый bundle установлен,
Linux control CI35503217768 success. Архив Linux требует ручной установки;
Windows физическая приёмка отложена. [Отчёт публикации](releases/2026-09-20-three-platform-downloads.ru.md).
Предложение по росту: [реестр шлюзов и распределение назначений](server-capacity-plan.ru.md).
Балансировка по нагрузке пока не реализована; новые VPS не заказывались.

Этап 5 (независимый служебный канал) частично реализован; Friends через HTTPS
не закрывает Reticulum/native rollout. Для этапа 6 оба gateway существуют,
но независимость control ingress и смена полностью недоступного endpoint ещё
не приняты. Этап 7/коммерческий запуск не закрыт. AWG2 не считаем проверенным
российским транспортом: ориентир AWG3.1/TCP REALITY, проверка в российской сети впереди.
Длительная устойчивость и восстановление после смены сети/сна требуют приёмки.

[Платформы, проверки и откат](releases/2026-09-20-desktop-map-dial.ru.md).
