...
## Следующее обновление клиентов

Android beta38 и настольная версия 0.2.10 проходят проверку на Android, Windows и Linux. Доступ остаётся только по приглашению: установите приложение со страницы приглашения, затем нажмите на ней **«Открыть приложение»**. Приложение само обменивает приглашение на доступ для устройства — вводить и копировать ключ не потребуется. Индивидуальные ключи сохраняются для управления доступом и его отзыва.

Страну и протокол можно будет выбирать на месте подписей в панели соединения рядом с батареей. Переходы остаются внутри основного окна; образец общего оформления — Android. В публичных загрузках этих изменений пока нет: [проверки и состояние выпуска](docs/releases/2026-09-23-public-client-status.ru.md).

## Платформы

| Платформа | Что доступно сейчас |
| --- | --- |
| Android 8+ | Beta35 по приглашению; ARM64 APK, 36,4 МБ. VPN и текстовая переписка проверены на телефонах. |
| Linux | Пилотный клиент GTK 4 / libadwaita; архив 0.2.9 preview146d221d0ad23c07. Настройка с помощью оператора. |
| Windows x64 | Нативный пилотный клиент; установщик 0.2.9 previewa6c68fe. Активация через оператора; доверенной подписи издателя пока нет. |
...
Тестовая Android beta38, русский интерфейс, 23 сентября 2026 года. Настоящий нативный экран с устройства; на кадре VPN выключен. В публичных загрузках пока beta35. [Происхождение изображения](docs/assets/README.md).
...
**Linux и Windows:** [инструкции настольного пилота](docs/clients.ru.md) и [актуальные preview](docs/releases.md). Для активации сейчас нужна помощь оператора.
...
- [Android beta и контрольная сумма](docs/releases/2026-09-23-public-client-status.ru.md)
...
**Состояние исходников:** публичная ветка main пока отстаёт от распространяемой Android beta35. Возможности выше относятся к проверенной APK; сборка main не воспроизводит эту версию. Публикация соответствующего состояния исходников включена в [план](docs/PLAN.md).
