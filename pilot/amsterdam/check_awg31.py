"""Bounded real Amsterdam AWG3.1 checks in an isolated Linux network namespace.

Root only. AWG engine/UDP sockets stay in the original network namespace; all test
...
NS='fc-awg31-check';IFACE='fc31chk';HOST='186.246.45.246'
...
def main():
    ...
    parser.add_argument('--awg',type=Path,required=True)
    a=parser.parse_args()
    assert os.geteuid()==0
    state=a.state.resolve();owner=(state/'client.key').stat().st_uid;user=pwd.getpwuid(owner).pw_name
    wg=str(a.awg.resolve());assert (state/'client-uapi.conf').is_file()
    ...
    engine=None
    ipc=Path('/run/amneziawg')/(IFACE+'.sock');assert not ipc.exists()
    def stop_engine():
        nonlocal engine
        if engine is not None:
            if engine.poll() is None:engine.terminate()
            engine.wait(timeout=10);engine=None
    def connect():
        nonlocal engine
        engine=subprocess.Popen([str(a.engine),'-f',IFACE],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,env=os.environ|{'LOG_LEVEL':'silent','GOMAXPROCS':'1'})
        deadline=time.monotonic()+5
        while not ipc.exists():
            if engine.poll() is not None or time.monotonic()>deadline:raise RuntimeError('Engine IPC startup failed')
            time.sleep(.02)
        run(wg,'setconf',IFACE,str(state/'client-uapi.conf'))
        run('ip','link','set',IFACE,'netns',NS)
        inside('ip','link','set','lo','up');inside('ip','address','add',f'10.80.0.{a.peer_index}/32','dev',IFACE)
        inside('ip','-6','address','add',f'fd80:92::{a.peer_index:x}/128','dev',IFACE)
    ...
    try:
        ...
        connect()
        stage='gateway_ping';ping=inside('ping','-n','-c','3','-W','3','10.80.0.1')
        ...
        assert blocked.returncode!=0;result['tests'].append(dict(test=stage,passed=True))
        result['server_restart_tested']=False
        stage='client_reconnect';stop_engine();connect()
        start=time.monotonic()
        while time.monotonic()-start<35:
            ready=inside('ping','-n','-c','1','-W','1','10.80.0.1',check=False,timeout=3)
        ...
        result['ipc_removed']=not ipc.exists()
        result['finished_at']=int(time.time())
        result['passed'] &= all(result[k] for k in ('host_routes_rules_unchanged','host_resolver_unchanged','namespace_removed','interface_removed','ipc_removed'))
