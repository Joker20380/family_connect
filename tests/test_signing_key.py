...
def test_real_vault_signs_update_without_plaintext_key_file(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr('sys.argv',['sign_update','--vault',str(vault),'--vault-entry','test',
        '--vault-member','keys/root','--vault-public-key',str(anchor),'--vault-password-dialog',
        '--platform','windows','--version','0.2.13','--sequence','99','--artifacts',str(tmp_path),'--output',str(output)])
    sign_update.main()
    envelope=json.loads(output.read_text());payload=base64.b64decode(envelope['payload'])
    private.public_key().verify(base64.b64decode(envelope['signature']),sign_update.WINDOWS_DOMAIN+payload)
    ...
    with pytest.raises(ValueError,match='private diagnostics suppressed'):sign_update.main()
